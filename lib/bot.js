...
async function setWebhook(webhookUrl) {
  const bot = getBot();
  if (!bot) return { ok: false, error: 'no_token' };

  // Include my_chat_member so we learn group id when bot is added as admin
  try {
    ...
    // Some telegram-bot-api versions only accept url
...
async function diagnoseBot() {
  ...
  const result = {
    ...
    group_source: process.env.GROUP_CHAT_ID && process.env.GROUP_CHAT_ID !== '0' ? 'env' : gid ? 'db/cache' : null,
    bot: null,
  ...
  try {
    // telegram-bot-api may expose getMe differently
    const me = await bot.getMe();
    result.bot = me;
  } catch (e) {
  ...
  if (!gid) {
    ...
    try {
      ...
      result.group_send_test = { ok: true, message_id: r?.message_id || r?.result?.message_id };
    } catch (e) {
      result.group_send_test = { ok: false, error: e.message };
      result.tips.push(
