  ...
  return {
    execute: async (query, params = []) => {
      ...
      try {
        ...
        // ─── app_settings key/value (group_chat_id etc.) ───
        if (upper.includes('FROM APP_SETTINGS')) {
          const [key] = params;
          const { data, error } = await getSupabase()
            .from('app_settings')
            .select('key, value')
        ...
        if (
          upper.includes('INSERT OR REPLACE INTO APP_SETTINGS') ||
          upper.includes('INSERT INTO APP_SETTINGS')
        ) {
        ...
        // ─── INSERT INTO driver_tracks ───
        if (upper.includes('INSERT INTO DRIVER_TRACKS')) {
          // params: [driver_id, driver_name, phone, lat, lng, accuracy, speed, heading, created_at?]
          const [driver_id, driver_name, phone, lat, lng, accuracy, speed, heading, created_at] =
            params;
          const row = {
            driver_id,
        ...
        // ─── SELECT latest track points (all drivers or one) ───
        if (upper.includes('FROM DRIVER_TRACKS')) {
          let q = getSupabase().from('driver_tracks').select('*');
          // WHERE driver_id = ?
          if (upper.includes('WHERE DRIVER_ID =')) {
            const [driver_id] = params;
            q = q.eq('driver_id', driver_id);
          }
          // optional time window: created_at >= ?
